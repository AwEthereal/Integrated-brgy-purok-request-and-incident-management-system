...
<body>
    <div class="header">
        ...
        <div class="report-title">LIST OF RESIDENTS</div>
        <div class="report-meta">
            Generated on: {{ now()->format('F j, Y h:i A') }}<br>
            Total Records: {{ $residents->count() }}
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ strtoupper($resident->last_name) }}, {{ strtoupper($resident->first_name) }} {{ strtoupper($resident->middle_name) }}</td>
                ...
                <td>{{ $resident->is_approved ? 'Active' : 'Inactive' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
