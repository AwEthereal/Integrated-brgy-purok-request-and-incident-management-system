...
@section('title', 'Add Official Account')

@section('content')
<div class="py-6">
  <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-semibold mb-4">Add Official Account</h1>

    <form method="POST" action="{{ route('captain.secretaries.store') }}" class="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-xl shadow border border-gray-200 dark:border-gray-700">
      @csrf
      <div>
      ...
      <div class="flex items-start gap-2">
        <input id="encodeRbi" type="checkbox" name="encode_rbi" value="1" @checked(old('encode_rbi', true)) class="mt-1 rounded border-gray-300 text-purple-600 shadow-sm focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50">
        <label for="encodeRbi" class="text-sm text-gray-700 dark:text-gray-300">
          Encode RBI Form B for this account after saving
        </label>
      </div>
      ...
      <div class="flex items-center justify-end gap-2">
        ...
        <button class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500">Save</button>
      </div>
    </form>
