    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        @if ($requests->isEmpty())
            <div class="p-6 text-gray-500 text-center">
                No pending purok clearance requests at the moment.
            </div>
        @else
                        ...
                        @foreach ($requests as $request)
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @if ($request->remarks)
                                        <div class="text-sm text-gray-500">{{ $request->remarks }}</div>
                                    @endif
                                </td>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form id="approveForm" method="POST" action="">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4" id="modal-title">Approve Request</h3>
                    <div class="mt 2">
                        <label for="purok_notes" class="block text-sm font-medium text-gray-700">Notes (Optional)</label>
                        <textarea name="purok_notes" id="purok_notes" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                    </div>
                </div>
...
<script>
    function openApproveModal(requestId) {
        ...
        form.action = `/requests/${requestId}/approve-purok`;
